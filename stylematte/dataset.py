...
bg_files = sorted(glob.glob(config["bg"]+"/*.jpg"))
#p3files = list(glob.glob(config["p3m10k"]+"/train/blurred_image/*.jpg"))+list(glob.glob(config["p3m10k"]+"/train/mask/*.png"))
p3files = list(glob.glob(config["p3m10k"]+"/train/foregrounds/*.png"))
...
class P3M10KBlurred(data.Dataset):
    def __init__(self, bg_files=bg_files, dataset_path=config["p3m10k"], transform="hard"):
...
class AM2K(data.Dataset):
    def __init__(self, is_train=True, fake_bg=True):
...
if __name__ == '__main__':
    ...
    input_dict = ds[random.randint(0, len(ds))]
