...
_background = np.array(Image.open("background.png"))
...
def prepare_texture_data(image):
    ...
    if image.shape[2] == 4 :#we don't find how to render alpha  
        mask = (image[:,:,3:4])/255.
        foreground = image[:,:,:3] #extract foreground?
        if _background.shape!=foreground.shape:
            print("resize")
            _background = np.array(Image.open("background.png").resize((foreground.shape[0],foreground.shape[1])))

        image = np.uint8((foreground*mask +(1-mask)*_background).clip(0,255))
